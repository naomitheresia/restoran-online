...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($items as $menu_id => $qty) {
        ...
        $selected[] = ['menu' => $m, 'qty' => $qty, 'subtotal' => $subtotal];
        $total += $subtotal;
    }
    ...
    if (empty($errors)) {
        ...
        foreach ($selected as $it) {
            $stmt = $conn->prepare('INSERT INTO detail_transaksi (transaksi_id,menu_id,harga,qty,subtotal) VALUES (?,?,?,?,?)');
            $stmt->bind_param('iiidi', $order_id, $it['menu']['id'], $it['menu']['harga'], $it['qty'], $it['subtotal']);
            $stmt->execute();
                ...
                <tbody>
                    <?php while ($m = $menu_res->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= $m['stok'] ?></td>
                            <td><input type="number" name="items[<?= $m['id'] ?>]" value="0" min="0" max="<?= $m['stok'] ?>"></td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
