...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['gambar']) && $_FILES['gambar']['error'] === 0) {
        ...
        if (!in_array(strtolower($ext), $allowed)) {
            ...
            if (move_uploaded_file($_FILES['gambar']['tmp_name'], $dst)) {
                if ($data['gambar'] && file_exists(__DIR__ . '/../' . $data['gambar'])) {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare('UPDATE menu SET kode=?,nama=?,kategori_id=?,harga=?,stok=?,deskripsi=?,gambar=? WHERE id=?');
        $stmt->bind_param('ssiddssi', $kode, $nama, $kategori_id, $harga, $stok, $deskripsi, $gambar, $id);
        
        if ($stmt->execute()) {
            flash('msg', 'Menu diperbarui');
...
<body>
    ...
    <div class="container card">
        ...
        <form method="post" enctype="multipart/form-data">
            <label>Kode</label>
            <input name="kode" value="<?= htmlspecialchars($data['kode']) ?>" required>
            ...
            <select name="kategori_id">
                ...
                <?php while ($c = $cats->fetch_assoc()): ?>
                    <option value="<?= $c['id'] ?>" <?= ($c['id'] == $data['kategori_id'] ? 'selected' : '') ?>>
            ...
            <label>Harga</label>
            <input name="harga" type="number" step="100" value="<?= $data['harga'] ?>" required>
            
            <label>Stok</label>
            <input name="stok" type="number" value="<?= $data['stok'] ?>">
            ...
            <?php if ($data['gambar']): ?>
                <p>Gambar sekarang: <img src="/restoran_fix_v2/<?= htmlspecialchars($data['gambar']) ?>" style="max-height:80px"></p>
            <?php endif; ?>
